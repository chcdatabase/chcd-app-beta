    ...
    return (
        ...
        {filteredData.map((item, index) => {
          if (item.value._type === "Person") {
            ...
            Nationality: {item.value.nationality}<br/>
            Birth Year: {item.value.birth_year}<br/>
            Death Year: {item.value.death_year}</div>
            <div className="itemColumn50">
            <b>Chinese Name:</b> {item.value.family_name_zh} {item.value.given_name_zh}<br/>
            Gender: {item.value.gender}<br/>
            Born in: {item.value.birth_city}<br/>
            Died in: {item.value.death_city}</div></div></div>
          }
